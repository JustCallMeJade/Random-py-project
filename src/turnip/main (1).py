...
def run(cmd, *, cwd=None, env=None, check=True, silent=False):
...
def install_dependencies():
    ...
    # Match bash: sed -i '/^Types:/ s/deb$/deb deb-src/' /etc/apt/sources.list.d/debian.sources
    sources = Path("/etc/apt/sources.list.d/debian.sources")
    if sources.exists():
        new_lines = []
        for line in sources.read_text().splitlines(keepends=True):
            stripped = line.rstrip("\r\n")
            if stripped.startswith("Types:") and stripped.endswith("deb"):
                line = stripped + " deb-src\n"
            new_lines.append(line)
        sources.write_text("".join(new_lines))

    run(["apt-get", "update"])
    run(["apt-get", "build-dep", "mesa",       "-y", "-qq"], silent=True)
    run(["apt-get", "build-dep", "libarchive", "-y", "-qq"], silent=True)

    # -qq comes after package names, matching bash line 24
    run([
        "apt-get", "install", "-y",
...
def download_ndk_and_mesa():
    print("installing NDK and Cloning Mesa's latest source")
...
def write_cross_files():
    mesa_dir = WORKDIR / "mesa"
    print("setting crossfiles and setting up mesa...")
...
def main():
    print("Only works in debian Arm64!!! press Ctrl + C to exit")
